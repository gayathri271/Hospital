...
function HospitalForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setHospitalId(response.data.hospital.id); // Store hospitalId
      alert(`Hospital registered successfully! Hospital ID: ${response.data.hospital.id}`);
      setFormData({ name: '', location: '' });
      setError(null);
    } catch (error) {
      setError('Error registering hospital: ' + (error.response?.data?.message || 'Unknown error'));
    ...
    <div className="container">
      ...
      {hospitalId && (
        <div className="success-message">
          Hospital registered with ID: <strong>{hospitalId}</strong>
        </div>
      )}
      <form className="form-container" onSubmit={handleSubmit}>
        <div className="form-group">
